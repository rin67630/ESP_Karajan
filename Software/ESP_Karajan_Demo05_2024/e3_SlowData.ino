...
#ifdef A0_IS_DEMO                                                    //DEMOating a solar panel from LDR of a Witty.
  payload.BatI = float(map(A0Raw, 15, 1023, 0, 100 * A0_MAX)) / 150;  // Amperes DEMOated Panel
  if (A0Raw > 200) 
  {
...
/*
  ...
  {
    ADC_IinRaw = ADC_IoutRaw * ADC_VoutRaw/ADC_VinRaw * 1.1;  // Input current is DEMOated with 90% efficiency
